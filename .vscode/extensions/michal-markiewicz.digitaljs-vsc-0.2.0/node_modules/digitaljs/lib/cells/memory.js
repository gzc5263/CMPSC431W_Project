...
// Memory cell
const Memory = _base.Box.define('Memory', {
  /* default properties */
  bits: 1,
  abits: 1,
  rdports: [],
  ...
  operation(data) {
    ...
    const do_read = (portname, port) => {
      ...
      for (const [num, wrport] of this.get('wrports').entries()) {
        ...
        if ('transparent' in port && mask_ok(port.transparent, num) && port_active(wrportname, wrport) && data[portname + 'addr'] == data[wrportname + 'addr']) out[portname + 'data'] = write_value(wrportname, wrport, out[portname + 'data'], data[wrportname + 'data']);
        if ('collision' in port && mask_ok(port.collision, num) && port_active(wrportname, wrport) && data[portname + 'addr'] == data[wrportname + 'addr']) out[portname + 'data'] = write_value(wrportname, wrport, out[portname + 'data'], _vl.Vector3vl.xes(bits));
      }
    };
...
const MemoryView = _base.BoxView.extend({
  _autoResizeBox: true,
  events: {
  ...
  _displayEditor(evt) {
    ...
    div.append((0, _jquery.default)('<div class="btn-toolbar" role="toolbar">' + '<div class="btn-group mr-2" role="group">' + '<button name="prev" type="button" class="btn btn-secondary" title="Previous page">←</button>' + '<button name="next" type="button" class="btn btn-secondary" title="Next page">→</button>' + '</div>' + //            '<div class="btn-group mr-2" role="group">' +
    ...
    //            '</div>' + 
    '<div class="input-group">' + help.baseSelectMarkupHTML(display3vl, this.model.get('bits'), 'hex') + '</div>' + '</div>' + '<table class="memeditor">' + '</table>'));
